...
public class ExcelSaveData {
	...
	public void SaveACC(Workbook workbook, ResultSet ACCdata, String ACCname, String reportName) {
		try {
			...
			HSSFRichTextString richTextStringRow1 = new HSSFRichTextString(
					"Report Name : " + reportName);
			HSSFRichTextString richTextStringRow2 = new HSSFRichTextString("Report Period (from-date, to-date):");
	...
	public void SaveHierarchy(String account) {
		...
		if (connection != null) {
			try {
				...
				while (hier.next()) {
					if (workbook.getSheet(hier.getString(1)) != null)
						continue;
					ResultSet acc = dataProvider.MonthEntriesData(hier.getString(1));
					SaveACC(workbook, acc, hier.getString(1), "Daily");
				}
			...
			try {
				...
				while (accountsForDailyReport.next()) {
					ResultSet accNumberData = dataProvider
							.getAccountNumberForDailyReport(accountsForDailyReport.getString(2));
					...
					if (prevACChier == accountsForDailyReport.getInt(1)) { // write data to the same hierarchy
						...
						SaveACC(workbook, accData, accNumber, "Daily");
